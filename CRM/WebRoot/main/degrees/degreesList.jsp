  ...
  <head>
    ...
    <script type="text/javascript">
    	$(function(){
			...
			$(pager).pagination({
				...
				onBeforeRefresh:function(p,s){
					location.href="${pageContext.request.contextPath }/degreesServlet?pager.cur_page="+p+"&pager.pageRow="+s;
				},
    	...
    	function edit(){
			...
			url = "${pageContext.request.contextPath }/degreesServlet?action=update&degreeid="+arr;
			location.href=url;	
		}
		...
		//新增
		function add(){
			url = "${pageContext.request.contextPath }/main/degrees/degreesAdd.jsp";
			location.href=url;
		}
		...
		function del(){
			...
			var url = "${pageContext.request.contextPath }/degreesServlet?action=del&degreeid="+arr;
			location.href=url;	
		}
  ...
  <c:if test="${dep eq 1}">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" BACKGROUND="images/maintitle.jpg" class="tablelist2">
  	...
  	<table class="easyui-datagrid" border="1" pagination="true"  id="changesize" style="width:auto;padding:0px"  singleSelect="false"  datapagesize="onClickRow:getRowData" >
    	<thead>
    		<tr>
    			<th field="id" checkbox="true"  width=80>岗位编号</th>
    			<th field="depname" width=70 align="center">岗位名称</th>
    		</tr>
    	</thead>
    	<tbody>
    	<!--  
    		...
    		var:变量名称，对应集合中的当前元素
    	-->
    	 <c:forEach items="${degreesList}" var="d">
    		<tr>
    			<td align="center">${d.degreeid}</td>
    			<td align="center">${d.degreename}</td>
    		</tr>
    	</c:forEach>
    	</tbody>
    </table>
